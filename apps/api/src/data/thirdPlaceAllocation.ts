const thirdPlaceAllocationColumns = ["A", "B", "D", "E", "G", "I", "K", "L"] as const;
...
type ThirdPlaceAllocationColumn = (typeof thirdPlaceAllocationColumns)[number];

export type ThirdPlaceAllocation = Partial<Record<ThirdPlaceAllocationColumn, string>>;

const thirdPlaceAllocationByQualifiedGroups = new Map<string, ThirdPlaceAllocation>(
  thirdPlaceAllocationRows.map(([qualifiedGroups, assignments]) => [
    qualifiedGroups,
    Object.fromEntries(
      assignments
        .split(",")
        .map((group, index) => [thirdPlaceAllocationColumns[index], group]),
    ) as ThirdPlaceAllocation,
  ]),
);

export function getThirdPlaceAllocation(qualifiedGroups: string[]) {
  return thirdPlaceAllocationByQualifiedGroups.get([...qualifiedGroups].sort().join("")) ?? null;
}
