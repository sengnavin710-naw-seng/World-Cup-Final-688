...
- แสดง `<displayName>` ใต้ชื่อทีมทั้งฝั่งเหย้าและฝั่งเยือนบนการ์ด Desktop โดยไม่เติมคำว่า `Owner:`
...
`AppShell` โหลดข้อมูล knockout และ teams ตามเดิม แล้วส่ง `teams` เข้า `KnockoutTab` โดยไม่เพิ่ม API request ใหม่ ภายใน `KnockoutTab` ฟังก์ชัน resolve ทีมจะอ่าน `ownedByName` จากทีมที่ตรงกับรหัสบน bracket และส่วนแสดงชื่อทีมบน Desktop จะแสดงค่า display name โดยตรง

## สถานะข้อมูล

- ทีมที่ resolve ได้และมี `ownedByName`: แสดง `<displayName>`
...
- Component test ยืนยันว่าการ์ด Desktop แสดง `<displayName>` โดยไม่มีคำว่า `Owner:` เมื่อทีมมีเจ้าของ
...
บนหน้า Knockout เวอร์ชัน Desktop การ์ดของทีมที่ถูกเลือกแสดง `<displayName>` ใต้ชื่อทีมอย่างอ่านได้โดยไม่มี prefix โดยชื่อยาวไม่ทำให้ layout เสีย และทีมที่ไม่มีเจ้าของหรือยังเป็น placeholder ไม่แสดงชื่อ
